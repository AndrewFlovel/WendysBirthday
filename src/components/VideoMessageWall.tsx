...
export const VideoMessageWall: React.FC<VideoMessageWallProps> = ({
  ...
  return (
    <section id="seccion-videos" className="relative z-10 py-12 px-4 max-w-6xl mx-auto">
      ...
      {/* Galería de Videos o Estado Vacío */}
      {filteredVideos.length > 0 ? (
        ...
        <div className="p-8 sm:p-10 rounded-3xl glass-panel border border-sky-400/40 text-center space-y-4 shadow-xl">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-sky-500 to-pink-500 flex items-center justify-center mx-auto shadow-md">
            <Video className="w-7 h-7 text-white" />
          </div>
          <h3 className="text-base sm:text-lg font-bold text-white font-display">
            ¡Sé el primero en felicitar a Wendy en video! 🎥✨
          </h3>
          <p className="text-xs sm:text-sm text-slate-300 max-w-md mx-auto leading-relaxed">
            Aún no hay videos subidos. Graba o sube tu saludo sorpresa para que Wendy lo descubra en su cumpleaños.
          </p>
          <button
            onClick={onOpenUpload}
            className="px-6 py-2.5 rounded-full font-bold text-xs bg-gradient-to-r from-sky-400 via-pink-500 to-yellow-400 text-white shadow-lg hover:scale-105 transition-all inline-flex items-center gap-1.5"
          >
            <Plus className="w-4 h-4" />
            <span>Subir o Grabar Video Ahora 📲</span>
          </button>
        </div>
      )}
